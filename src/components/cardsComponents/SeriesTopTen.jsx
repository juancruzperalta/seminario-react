...
export const SeriesTopTen = () => {
  ...
  {topSerie.slice(0, 10).map((serie, i) => (
    <div className='flex flex-col mr-4 max-h-[580px]'>
    <div key={i} className='flex items-center mb-1 bg-[var(--bgSecondary)]/90 border border-gray-700 rounded-lg p-[2.2px] shadow-md hover:bg-[var(--bgSecondary)] transition-all duration-300 bg-gradient-to-t from-black/60 via-black/30 to-transparent'>
      <img key={serie.id} src={`https://image.tmdb.org/t/p/w500${serie.backdrop_path}`} className='w-12 h-12 rounded-md p-1 object-cover' alt={serie.name} />
      <span><span className='text-yellow-300'>#{i+1}</span> {serie.name}</span>
    </div>
    </div>
  ))}
        </div>
    ...
    <button
            ...
            onClick={() => scrollButton(-serieRef.current.querySelector('img').clientWidth - 16)}> 
