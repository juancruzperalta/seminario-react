  ...
  return (
    <>
      {selectedId && (
        ...
        <button className='top-0 right-0 absolute bg-[var(--colorAccent)] opacity-80 hover:bg-[var(--buttomActive)] h-10 w-10 cursor-pointer rounded-full' onClick={() => setSelectedId(null)}>X</button>
        </div>
          </div>
      )}
    <div className='grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4 relative'>
      {movies.map((movie) => (
        ...
        <img key={movie.id} src={`https://image.tmdb.org/t/p/w500${movie.backdrop_path}`} alt={movie.name} className='h-[340px] object-cover rounded-lg shadow-md'/>
            <div className='absolute bottom-0 rounded-b-md w-full opacity-[98%] pt-2 bg-[var(--bgSecondary)]'><span>{movie.name}</span>
            ...
            <button className='cursor-pointer opacity-100 w-full p-1 font-medium bg-[var(--bgColor)] hover:bg-[var(--buttomActive)] ' onClick={() => viewTrailer(movie.id)}  >Trailer</button>
            </div>
        </div>
      </div>
      ))
