    ...
    <div className='grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4'>
      {movies.map((movie) => (
        ...
        <img key={movie.id} src={`https://image.tmdb.org/t/p/w500${movie.backdrop_path}`} alt={movie.name} className='h-[340px] object-cover rounded-lg shadow-md'/>
            <div className='absolute bottom-0 rounded-b-md w-full bg-gray-700 pt-2'><span>{movie.name}</span>
            ...
            <button className='cursor-pointer bg-gray-800 w-full p-1 font-medium' onClick={() => viewTrailer(movie.id)}>Trailer</button>
            </div>
      </div>
          </div>
      ))
